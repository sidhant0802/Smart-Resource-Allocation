...
from services.ml.scorer import UrgencyScorer
...
class MLAnalyzer:
    """
    ML analysis orchestrator — used as fallback when Gemini unavailable.
    Coordinates all ML sub-modules.
    """

    @classmethod
    def extract_keywords(cls, text: str, matched: List[str]) -> List[str]:
        """Combine matched keywords + frequent meaningful words"""
        found = list(matched)
        ...
        stop_words = {
            ...
            "would", "could", "should", "there", "here", "each",
            "other", "such", "only", "both", "many", "most", "much",
        }
    ...
    def analyze(cls, text: str, has_file: bool = False) -> dict:
        ...
        if not text or len(text.strip()) < 10:
            return {
                ...
                "sentiment": "neutral",
                "sentiment_score": 0,
                "summary": "Insufficient text for analysis. Please provide more details.",
                "detailed_analysis": "",
                "key_problems": ["Insufficient content provided"],
                "suggested_actions": [
                    "Submit a more detailed report with specific information",
                    "Describe what you observed in detail",
                    "Include location, number of people affected, and timeline"
                ],
                "keywords": [],
                ...
                "model_used": "ml-pipeline-v5",
            }

        logger.info(f"🔧 ML Analyzing {len(text)} chars")
        ...
        logger.info(f"People: {affected_people} | Area: {affected_area}")
        ...
        try:
            ...
            score = 25.0
            severity = "low"
            explanation = f"Fallback scoring (error: {str(e)[:50]})"
            matched_kw = []

        # Step 5: Keywords
        keywords = cls.extract_keywords(text, matched_kw)

        # Step 6: Immediate Risk
        immediate_risk = (
            score >= 75 or
            severity == "critical" or
            indicators.get("has_deaths", False) or
            (indicators.get("has_disease", False) and score >= 55) or
            indicators.get("has_violence", False) and score >= 65
        )
        ...
        confidence = min(
            0.88,
            cat_conf + 0.15 + (0.08 if has_file else 0) + (0.05 if affected_people else 0)
        )

        processing_time = round(time.time() - start, 3)

        logger.info(
            f"✅ ML Analysis done | Score: {score} | "
